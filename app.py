...
from sessions import track_sessions
...
if mode == "CSV Lokal":
    ...
    jobs = fetch_all_jobs(limit_total=None, per_page=200)  # ambil semua data
    if not jobs:
...
for col in ["perusahaan.nama_perusahaan", "perusahaan.nama_kabupaten",
            "perusahaan.nama_provinsi","perusahaan.alamat"]:
...
if "posisi" in df.columns:
    posisi = st.sidebar.multiselect("Filter Posisi", df["posisi"].dropna().unique())
    if posisi:
...
if "jumlah_kuota" in df.columns and df["jumlah_kuota"].notna().any():
    ...
    if kuota_min != kuota_max:
        kuota_min, kuota_max = st.sidebar.slider(
            "Filter Jumlah Kuota", kuota_min, kuota_max, (kuota_min, kuota_max)
        )
...
if "jumlah_terdaftar" in df.columns and df["jumlah_terdaftar"].notna().any():
    ...
    if terdaftar_min != terdaftar_max:
        terdaftar_min, terdaftar_max = st.sidebar.slider(
            "Filter Jumlah Terdaftar", terdaftar_min, terdaftar_max, (terdaftar_min, terdaftar_max)
        )
...
if not df.empty:
    ...
    st.plotly_chart(fig1, use_container_width=True)  # tetap bisa dipakai, warning sampai akhir 2025
...
# 🔧 Tambahkan opsi jumlah baris per halaman
...
with col1:
    if not df.empty:
        st.download_button(
            "⬇️ Download Semua Hasil Filter (CSV)",
...
with col2:
    if not df_page.empty:
        st.download_button(
            f"⬇️ Download Data Halaman {page} (CSV)",
            df_page.to_csv(index=True).encode("utf-8"),
            f"jobs_page_{page}.csv",
            "text/csv"
        )
